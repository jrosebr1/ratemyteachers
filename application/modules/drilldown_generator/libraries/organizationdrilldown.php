...
class OrganizationDrillDown extends DrillDownDumper
{
	/**
	...
	public function dump()
	{
		...
		for ($enumAlpha = 0; $enumAlpha < strlen($alphabet); $enumAlpha++)
		{
			...
			else
			{
				...
				$sql .= "FROM " . $this->coreTables["Organizations"] . " ";
				$sql .= "WHERE " . $this->coreSchemas["Organizations"]["CountryID"] . "=? ";
				$sql .= "AND " . $this->coreSchemas["Organizations"]["Name"] . " LIKE ? ";
				$sql .= "ORDER BY " . $this->coreSchemas["Organizations"]["Name"] . " ASC ";
				$sql .= ") AS tbl;";
	...
	public function createDirStructure()
	{
		...
		@mkdir($this->dumpDir . "/organization");
		@mkdir($this->outputDir . "/organization");
		
		// construct the dump path and then ensure the specific site
		// directory exists
		$this->dumpPath = $this->dumpDir . "/organization/" . $this->countryCode;
		@mkdir($this->dumpPath);
		@mkdir($this->outputDir . "/organization/" . $this->countryCode);
	}
}
